...
//		osDelay(5);
		...
		if( ledPlayCnt > 500000 ) {
			...
			if( ledTempCnt > 200 ) {
				...
				fanOnFlg = 1 - fanOnFlg;
				if( fanOnFlg ) HAL_GPIO_WritePin(FAN_ON_GPIO_Port, FAN_ON_Pin, GPIO_PIN_SET);   // fan on
				else 		   HAL_GPIO_WritePin(FAN_ON_GPIO_Port, FAN_ON_Pin, GPIO_PIN_RESET); // fan off
